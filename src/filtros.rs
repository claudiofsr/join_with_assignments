...
pub fn codigo_nat_01() -> PolarsResult<Expr> {
    let codigo_bens_para_revenda: [u32; 1] = [1];
    let natureza: &str = coluna(Left, "natureza");
    let series: Series = get_series(codigo_bens_para_revenda);
    let literal_series: Expr = series.to_list_expr()?;
...
pub fn aliquota_de_receita_financeira() -> Expr {
    ...
    let aliq_pis: Expr = col(aliquota_de_pis)
        .is_not_null()
        .and((col(aliquota_de_pis) - lit(0.65)).abs().lt(lit(0.0001)));
    let aliq_cof: Expr = col(aliquota_de_cof)
        .is_not_null()
        .and((col(aliquota_de_cof) - lit(4.0)).abs().lt(lit(0.0001)));
...
///
/// Evita falhas silenciosas de precisão de ponto flutuante (f64) aplicando uma
/// tolerância de 0.0001 para as alíquotas de PIS (0,65%) e COFINS (3,00%).
pub fn aliquota_de_receita_cumulativa() -> Expr {
    let aliquota_de_pis: &str = coluna(Left, "aliq_pis"); // "Alíquota de PIS/PASEP (em percentual)"
    let aliquota_de_cof: &str = coluna(Left, "aliq_cof"); // "Alíquota de COFINS (em percentual)"

    // Comparação matemática segura: |valor_lido - valor_esperado| < 0.0001
    let aliq_pis: Expr = col(aliquota_de_pis)
        .is_not_null()
        .and((col(aliquota_de_pis) - lit(0.65)).abs().lt(lit(0.0001)));
    ...
    let aliq_cof: Expr = col(aliquota_de_cof)
        .is_not_null()
        .and((col(aliquota_de_cof) - lit(3.0)).abs().lt(lit(0.0001)));
