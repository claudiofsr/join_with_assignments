...
/// Format worksheet
fn format_worksheet(df: &DataFrame, worksheet: &mut Worksheet, sheet_name: &str) -> PolarsResult<()> {
...
fn auto_color(df: &DataFrame, worksheet: &mut Worksheet, sheet_name: &str) -> PolarsResult<()> {
    ...
    let format_saldoc: Format = Format::new()
    ...
    let format_bcsoma: Format = Format::new()
        .set_background_color(XlsxColor::RGB(color_bcsoma));

    let color_debito: u32 = u32::from_str_radix("ccc0da", radix).unwrap();
    let format_debito: Format = Format::new()
        .set_background_color(XlsxColor::RGB(color_debito));
            ...
            for (row_num, data) in series.iter().enumerate() {
                if let Some(text) = data.get_str() {
                    if text.contains("(Soma)") {
                        worksheet.write_with_format(row_num as u32 + 1, col_num as u16, text, &format_bcsoma)?;
                    }
                    else if text.contains("Débitos:") {
                        worksheet.write_with_format(row_num as u32 + 1, col_num as u16, text, &format_debito)?;
                    }
                    else if text.contains("Saldo de Crédito") {
                        worksheet.write_with_format(row_num as u32 + 1, col_num as u16, text, &format_saldoc)?;
