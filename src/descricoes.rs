...
static NATUREZA_DA_BASE_DE_CALCULO_DOS_CREDITOS: Lazy<HashMap<i64, &'static str>> = Lazy::new(|| {
    HashMap::from([
        ...
        (91, "Débitos: Revenda de Mercadorias de NCM 2309.90 (PIS/PASEP)"),
        (95, "Débitos: Revenda de Mercadorias de NCM 2309.90 (COFINS)"),
...
fn tipo_descricao(series: Series) -> Result<Option<Series>, PolarsError> {
    ...
    let new_series: Series = series
        ...
        .map(|option_i64: Option<i64>|
            option_i64.map(|number| {
                match DESCRICAO_DO_TIPO_DE_CREDITO.get(&number) {
                    Some(&descricao) => descricao.to_string(),
                    None => format!("{number}: Sem descrição"),
                }
            })
...
fn tipo_mes(series: Series) -> Result<Option<Series>, PolarsError> {
    ...
    let new_series: Series = series
        ...
        .map(|option_i64: Option<i64>|
            option_i64.map(|number| {
                match DESCRICAO_DO_MES.get(&number) {
                    Some(&descricao) => descricao.to_string(),
                    None => format!("{number}: Sem descrição"),
                }
            })
...
pub fn descricao_da_natureza_da_bc_dos_creditos(series: Series) -> Result<Option<Series>, PolarsError> {
