...
pub trait Extensions {
    ...
    fn get_schema(&self, side: Side) -> Schema;
...
mod tests {
    ...
    #[test]
    fn column_schema() {
        // cargo test -- --show-output column_schema

        let cols: Vec<Column> = Column::get_columns();

        let schema_left: Schema = cols.get_schema(Left);
        println!("schema_left: {schema_left:#?}");

        let schema_right: Schema = cols.get_schema(Right);
        println!("schema_right: {schema_right:#?}");
    }
