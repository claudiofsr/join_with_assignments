...
/// Safely casts a slice of elements of type `T` into a vector of type `U`.
///
/// # Examples
///
/// ```rust
    ...
    slice_a
        ...
        .map(|&a1| {
            slice_b
                ...
                .map(|&a2| {
                    let delta = (a1 - a2).abs();
                    let square_value = delta.powi(2).ceil();
                    square_value as i64
                })
                .collect()
        })
...
fn convert_to_square_matrix(matrix: &mut Vec<Vec<i64>>) {
    ...
    match row_number.cmp(&col_number) {
        Ordering::Less => {
            // Add rows
            // Se houver menos linhas do que colunas,
            // redimensiona a matriz adicionando linhas de zeros
            matrix.resize(col_number, vec![0; col_number]);
        }
        Ordering::Greater => {
            // Add columns
            // Se houver mais linhas do que colunas,
            // redimensiona cada linha de forma individual
            for row in matrix.iter_mut() {
                row.resize(row_number, 0);
            }
        }
...
/// Formats and displays the Kuhn-Munkres minimum cost assignment matrices.
fn show_assignments(
    vec_a: &[f64],
    vec_b: &[f64],
    matrix: &[Vec<i64>],
...
/// Determines the maximum text width needed to display elements of both slices.
...
/// Prints the formatted matrix values with terminal highlight colors.
fn print_matrix(
    matrix: &[Vec<i64>],
    array1: &[f64],
    array2: &[f64],
...
/// Renders a tabular structure into the terminal.
...
/// Renders the optimal mappings found by the algorithm.
...
/// Run tests with:
/// cargo test -- --show-output tests_munkres
#[cfg(test)]
mod tests_munkres {
    use super::*;
